...
export default function LiquiditySweepBacktestPage() {
  ...
  async function runBacktest() {
    ...
    try {
      const body = { days };
      if (minRr) body.minRr = parseFloat(minRr);
      if (debugMode) body.debug = true;
      const res = await fetch("/api/backtest-liquidity", {
        method: "POST",
  ...
  const summary = useMemo(() => {
    ...
    // average target RR (reward:risk to the liquidity) across all signals
    const rrVals = trades.map((t) => Number(t.rr)).filter((n) => !isNaN(n));
    const avgTargetRr = rrVals.length ? round(rrVals.reduce((a, n) => a + n, 0) / rrVals.length, 2) : 0;
    ...
    <main className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Engulfing at level · stop beyond swing pivot · target next liquidity{" "}
      ...
      {loading && !data && (
        <div className="mt-16 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          <Spinner />
          <p className="text-sm">Running liquidity-sweep backtest…</p>
        </div>
      )}
      ...
      {data && summary && (
        <div className="mt-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <StatCard label="Avg Target R:R" value={`${summary.avgTargetRr}R`} sub="to liquidity" />
