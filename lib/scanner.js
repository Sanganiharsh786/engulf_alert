...
// Liquidity Sweep alert text: engulfing at a level, stop beyond the swing
// pivot, target the next opposite-side liquidity.
function sweepAlertText(pair, direction, low, high, curr, plan) {
  const t = new Date(curr.ts).toISOString().replace("T", " ").slice(0, 16);
  return (
    `${direction.toUpperCase()} LIQUIDITY SWEEP setup\n` +
    `--------------------------------------\n` +
    `Pair      : ${pair.name} (${pair.symbol} @ ${pair.exchange})\n` +
    `Level     : ${round(low)} - ${round(high)}\n` +
    `Candle UTC: ${t}\n\n` +
    `Entry (engulfing close): ${round(plan.entry)}\n` +
    `Stop (beyond swing)    : ${round(plan.stop)}\n` +
    `Target (next liquidity): ${round(plan.tp)}\n` +
    `Reward:Risk            : ${round(plan.rr, 2)}R\n` +
    `\nTradingView: ${tradingViewLink(pair)}\n`
  );
}

// Scan the latest closed candle for a Liquidity Sweep setup and alert on it.
// Independent of the standard/zone-origin engulfing stream: its own dedupe key
// (strategy-tagged) and its own settings toggle.
export async function scanPairLiquiditySweep(pair, settings, store, { dryRun = false } = {}) {
  const tf = pair.timeframe || settings.timeframe || "15m";
  const cfg = sweepConfig(settings);
  // fetch enough history to confirm swings within the lookback window
  const raw = await fetchOHLCV(pair, tf, Math.max(60, cfg.lookback + 20));
  const rows = closedCandles(raw, tfSeconds(tf), Date.now());
  if (rows.length < 3) {
    return { pair: pair.name, tf, status: "waiting", strategy: "liquidity-sweep", alerts: [] };
  }
  ...
  const result = { pair: pair.name, tf, status: "ok", direction, strategy: "liquidity-sweep", alerts: [] };
  if (!direction) return result;
  ...
  for (const lvl of pair.levels) {
    ...
    if (!plan) continue; // no swing stop / next-liquidity target -> no setup

    const key = `${pair.name}|${tf}|${lvl.id}|${direction}|sweep|${curr.ts}`;
    if (store.alertedKeys.includes(key)) continue;
    store.alertedKeys.push(key);
    ...
    // override the fixed-RR take-profit with the actual liquidity target
    if (position) {
      const pip = Number(pair.pipSize) || 1;
      position.tp = round(plan.tp, 4);
      position.rr = round(plan.rr, 2);
      position.tpPips = round(plan.reward / pip, 1);
      position.rewardAmount = round(position.riskAmount * plan.rr, 2);
    }
    ...
    const text = sweepAlertText(pair, direction, low, high, curr, plan) + positionText(position);

    if (!dryRun && senderConfigured) {
      try {
        await sendAlertEmail(settings.email, {
          subject: `[${pair.name}] LIQUIDITY SWEEP ${direction.toUpperCase()} at ${round(low)}-${round(high)} (${round(plan.rr, 2)}R)`,
          text,
          svg,
          filename: `${pair.name}_${direction}_sweep.svg`,
        });
        alert.emailed = true;
      } catch (e) {
        alert.emailed = false;
    ...
    if (!dryRun && telegramConfigured) {
      const telegramOptions = {};
      if (settings.telegram && settings.telegram.chatId) telegramOptions.chatId = settings.telegram.chatId;
      if (settings.telegram && settings.telegram.botToken) telegramOptions.botToken = settings.telegram.botToken;
      try {
        await sendTelegramPhoto(svg, text, telegramOptions);
        alert.telegramSent = true;
      } catch (e) {
        try {
          await sendTelegramMessage(text, telegramOptions);
          alert.telegramSent = true;
          alert.telegramError = "Image failed, text sent: " + String(e.message || e);
        } catch (fallbackError) {
          alert.telegramSent = false;
...
export async function runScan(store, opts = {}) {
  const results = [];
  const sweepOn = !!(store.settings.liquiditySweepAlerts && store.settings.liquiditySweepAlerts.enabled);
  for (const pair of store.pairs) {
    ...
    if (sweepOn) {
      try {
        results.push(await scanPairLiquiditySweep(pair, store.settings, store, opts));
      } catch (e) {
        results.push({ pair: pair.name, status: "error", strategy: "liquidity-sweep", error: String(e.message || e) });
      }
    }
  }
