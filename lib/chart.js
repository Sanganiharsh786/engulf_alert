...
export function buildChartSVG({ pair, tf, rows, signalTs = null, direction = null, entry = null, stop = null, tp = null, levelLow = null, levelHigh = null }) {
  ...
  rows.forEach((r, i) => {
    ...
    // Wick (shadow line above and below the candle body)
    const wickTop = Math.min(yPrice(h), yPrice(l));
    const wickBot = Math.max(yPrice(h), yPrice(l));
    const wickH = wickBot - wickTop;
    if (wickH > 0.5) {
      // Draw wick as a thick vertical line from high to low
      s += `<line x1="${cx}" y1="${wickTop}" x2="${cx}" y2="${wickBot}" stroke="${col}" stroke-width="2" opacity="0.9"/>`;
    }
