...
export async function sendTelegramMessage(message, options = {}) {
  ...
  const payload = {
    chat_id: chatId,
    text: message,
    // Remove HTML parsing to match plain text email format
    disable_web_page_preview: false, // Allow TradingView preview
    ...options.extraParams
  };
...
export async function sendTelegramDocument(document, caption, options = {}) {
  ...
  const formData = new FormData();
  formData.append('chat_id', chatId);
  // Remove HTML parsing to match email format
  formData.append('caption', caption || '');
...
export function formatTelegramAlert(pair, direction, low, high, prev, curr, pos) {
  ...
  // Use exact same format as email alertText function
  let message = (
  ...
  // Add position info if available (exactly like email)
  if (pos) {
    message += `\nTRADE PLAN (risk ${pos.riskPercent}% of account)\n`;
    message += `--------------------------------------\n`;
    message += `Entry      : ${pos.entry}\n`;
    message += `Stop loss  : ${pos.stop}   (${pos.slPips} pips)\n`;
    message += `Take profit: ${pos.tp}   (${pos.tpPips} pips, ${pos.rr}R)\n`;
    message += `Lot size   : ${pos.lots} lots\n`;
    message += `  = risk ${pos.riskAmount} / (${pos.slPips} pips x ${pos.pipValuePerLot}/pip per lot)\n`;
    message += `Margin used: ${pos.margin}   @ ${pos.leverage}x leverage   (notional ${pos.notional})\n`;
    message += `Risk       : ${pos.riskAmount}   ->  Reward: ${pos.rewardAmount}\n`;
    
    if (pos.marginExceedsAccount) {
      message += `NOTE: required margin exceeds account — raise leverage or lower risk.\n`;
    }
  }

  // Add footer (exactly like email)
  message += `\n\nChart with the level + engulfing candle is attached.\n`;
  message += `TradingView: ${`https://www.tradingview.com/chart/?symbol=${pair.tradingview || `${pair.exchange.toUpperCase()}:${pair.name}`}`}`;
