// Run: node lib/fibStrategy.test.mjs
// Covers the multi-target / session / limits additions to the fib strategy.
import {
  fibConfig, activeTargets, fibTarget, nearestLiquidity, inSession,
  bullishStructureBreak, bearishStructureBreak, confirmLong,
  sessionKeyOfTs, SESSIONS,
} from "./fibStrategy.js";
import { backtestPair, computeStats } from "./fibBacktest.js";

let pass = 0, fail = 0;
const ok = (n, c) => { c ? pass++ : fail++; console.log((c ? "PASS" : "FAIL") + " - " + n); };

// --- config defaults --------------------------------------------------------
const c = fibConfig();
ok("multi-target on by default", c.useMultiTarget === true && c.targetMode === "ALL");
ok("partials 40/30/30", c.tp1Percent === 40 && c.tp2Percent === 30 && c.tp3Percent === 30);
ok("zone normalised", c.fibUpper === 0.7 && c.fibLower === 0.786);
ok("max active trades = 1", c.maxActiveTrades === 1);

// --- target modes -----------------------------------------------------------
ok("ALL enables 3", JSON.stringify(activeTargets({ targetMode: "ALL" })) === '{"rr":true,"liq":true,"fib":true}');
const rl = activeTargets({ targetMode: "RR_LIQ" });
ok("RR_LIQ excludes fib", rl.rr && rl.liq && !rl.fib);

// --- fib target (TP3) -------------------------------------------------------
ok("fib ratio 0 = swing high (long)", fibTarget("bullish", 4500, 4000, 0) === 4500);
ok("fib ratio .7 = 0.700 line (long)", Math.abs(fibTarget("bullish", 4500, 4000, 0.7) - 4150) < 1e-9);
ok("fib short mirrors", fibTarget("bearish", 4500, 4000, 0) === 4000);

// --- structure-break confirmation ------------------------------------------
const prev = { open: 10, high: 11, low: 9, close: 9.5 };
ok("bull structure break", bullishStructureBreak(prev, { open: 9.6, high: 12, low: 9.5, close: 11.5 }));
ok("no break if closes below prev high", !bullishStructureBreak(prev, { open: 9.6, high: 12, low: 9.5, close: 10.8 }));
ok("no break if bearish body", !bullishStructureBreak(prev, { open: 11.6, high: 12, low: 9.5, close: 11.2 }));
ok("bear structure break", bearishStructureBreak({ open: 10, high: 11, low: 9, close: 10.5 }, { open: 10.4, high: 10.5, low: 8, close: 8.5 }));
ok("confirmLong STRUCTURE mode", confirmLong(prev, { open: 9.6, high: 12, low: 9.5, close: 11.5 }, "STRUCTURE"));
ok("combo ENGULFING_OR_STRUCTURE", confirmLong(prev, { open: 9.6, high: 12, low: 9.5, close: 11.5 }, "ENGULFING_OR_STRUCTURE"));

// --- liquidity (TP2) --------------------------------------------------------
const highs = [{ index: 10, price: 120 }, { index: 20, price: 110 }, { index: 5, price: 105 }];
ok("nearest liq above entry", nearestLiquidity("bullish", 100, 25, highs, [], 60) === 105);
ok("lookback excludes old pivots", nearestLiquidity("bullish", 100, 25, highs, [], 10) === 110);
ok("none above -> null (no fake target)", nearestLiquidity("bullish", 200, 25, highs, [], 60) === null);
const lows = [{ index: 10, price: 80 }, { index: 20, price: 90 }];
ok("nearest liq below (short)", nearestLiquidity("bearish", 100, 25, [], lows, 60) === 90);

// --- sessions ---------------------------------------------------------------
// Windows are IST (UTC+5:30) and must match components/backtest/utils.js:
//   Sydney 02:30–05:30 · Tokyo 05:30–12:30 · London 12:30–17:30 · NY 17:30–02:30
// Helper takes IST hour:minute and converts to the epoch ms the engine sees.
const ist = (h, m = 0) => Date.UTC(2024, 0, 15, h, m) - 5.5 * 3600e3;

ok("session windows match utils", SESSIONS.map((s) => s.window).join(",")
  === "02:30–05:30,05:30–12:30,12:30–17:30,17:30–02:30");
ok("sessionKeyOfTs: 14:00 IST = london", sessionKeyOfTs(ist(14)) === "london");
ok("sessionKeyOfTs: 20:00 IST = newyork", sessionKeyOfTs(ist(20)) === "newyork");
ok("sessionKeyOfTs: 09:00 IST = tokyo", sessionKeyOfTs(ist(9)) === "tokyo");
ok("sessionKeyOfTs: 03:00 IST = sydney", sessionKeyOfTs(ist(3)) === "sydney");
ok("sessionKeyOfTs: boundary 12:30 IST = london", sessionKeyOfTs(ist(12, 30)) === "london");
ok("sessionKeyOfTs: boundary 17:30 IST = newyork", sessionKeyOfTs(ist(17, 30)) === "newyork");

ok("ALL always true", inSession(ist(3), { sessionFilter: "ALL" }));
// the regression: 20:00 IST is New York, NOT London
ok("LONDON excludes 20:00 IST", !inSession(ist(20), { sessionFilter: "LONDON" }));
ok("LONDON excludes 09:00 IST", !inSession(ist(9), { sessionFilter: "LONDON" }));
ok("LONDON includes 14:00 IST", inSession(ist(14), { sessionFilter: "LONDON" }));
ok("NEWYORK includes 20:00 IST", inSession(ist(20), { sessionFilter: "NEWYORK" }));
ok("NEWYORK wraps to 01:00 IST", inSession(ist(1), { sessionFilter: "NEWYORK" }));
ok("NEWYORK excludes 14:00 IST", !inSession(ist(14), { sessionFilter: "NEWYORK" }));
ok("TOKYO includes 09:00 IST", inSession(ist(9), { sessionFilter: "TOKYO" }));
ok("SYDNEY includes 03:00 IST", inSession(ist(3), { sessionFilter: "SYDNEY" }));
ok("LONDON_NY union", inSession(ist(14), { sessionFilter: "LONDON_NY" })
  && inSession(ist(20), { sessionFilter: "LONDON_NY" })
  && !inSession(ist(9), { sessionFilter: "LONDON_NY" }));
ok("TOKYO_LONDON union", inSession(ist(9), { sessionFilter: "TOKYO_LONDON" })
  && inSession(ist(14), { sessionFilter: "TOKYO_LONDON" })
  && !inSession(ist(20), { sessionFilter: "TOKYO_LONDON" }));
// filter and table label must agree for every session key
ok("filter agrees with label for all sessions", SESSIONS.every((s) =>
  inSession(ist(0), { sessionFilter: s.key.toUpperCase() }) === (sessionKeyOfTs(ist(0)) === s.key)));
const cs = { sessionFilter: "CUSTOM", customSessionStart: "22:00", customSessionEnd: "02:00" };
ok("custom (IST) wraps midnight", inSession(ist(23), cs) && inSession(ist(1), cs) && !inSession(ist(12), cs));
ok("unknown filter = no filtering", inSession(ist(9), { sessionFilter: "NOPE" }));

// --- end-to-end: synthetic candles through backtestPair ---------------------
// Build a clean LONG setup: down leg to a pivot low, impulse up to a pivot
// high, pullback into the 0.70–0.786 zone, engulfing confirmation, rally to
// all targets. pivotLeft/right = 2 to keep the series short.
function bar(ts, o, h, l, cl) { return [ts, o, h, l, cl, 1000]; }
const T0 = Date.UTC(2024, 0, 1);
const H = 3600e3;
const rows = [];
let t = 0;
const add = (o, h, l, cl) => rows.push(bar(T0 + (t++) * H, o, h, l, cl));

// descent: pivot low at 100 (index 4)
add(118, 119, 112, 113); add(113, 114, 108, 109); add(109, 110, 104, 105);
add(105, 106, 101, 102); add(102, 103, 100, 100.5); // pivot low @100
// impulse up: pivot high at 130 (index 9)
add(100.5, 107, 100.4, 106.5); add(106.5, 114, 106, 113.5); add(113.5, 121, 113, 120.5);
add(120.5, 127, 120, 126.5); add(126.5, 130, 126, 129); // pivot high @130
// pivot confirms 2 bars later; drift down toward the zone
add(129, 129.5, 124, 124.5); add(124.5, 125, 119, 119.5);
// zone for 100->130 is 106.42 (0.786) .. 109 (0.700): dip in, bearish then bullish engulfing
add(119.5, 120, 112, 112.5);
add(112.5, 113, 107.5, 108);                 // bearish candle inside zone
add(107.6, 112.5, 107, 112.2);               // bullish engulfing -> entry @112.2
// rally through all targets
add(112.2, 118, 112, 117.5); add(117.5, 124, 117, 123.5);
add(123.5, 131, 123, 130.5); add(130.5, 140, 130, 139.5);
add(139.5, 145, 139, 144); add(144, 150, 143, 149);

const cfg = fibConfig({
  pivotLeft: 2, pivotRight: 2, minimumImpulsePercent: 5,
  confirmationMode: "ENGULFING", zoneExpiryBars: 300,
});
const pair = { name: "TEST", timeframe: "1h" };
const { trades } = await backtestPair(pair, cfg, { rows });
ok("e2e: exactly one trade", trades.length === 1);
if (trades.length === 1) {
  const tr = trades[0];
  ok("e2e: long", tr.direction === "bullish");
  ok("e2e: has TP1/TP2/TP3 fields", "tp1" in tr && "tp2" in tr && "tp3" in tr);
  ok("e2e: TP1 = entry + 2R", tr.tp1 != null && Math.abs(tr.tp1 - (tr.entry + 2 * (tr.entry - tr.stop))) < 0.01);
  ok("e2e: TP3 = swing high (ratio 0)", tr.tp3 === 130);
  ok("e2e: outcome win", tr.outcome === "win");
  ok("e2e: TP1+TP3 hit flags", tr.tp1Hit === true && tr.tp3Hit === true);
  ok("e2e: blended r positive", tr.r > 0);
  const stats = computeStats([tr], cfg);
  ok("e2e: stats tp1HitRate 100", stats.tp1HitRate === 100);
  ok("e2e: stats slHitRate 0", stats.slHitRate === 0);
  ok("e2e: one month bucket", stats.monthly.length === 1 && stats.monthly[0].month === "2024-01");
  ok("e2e: month labelled", stats.monthly[0].label === "Jan 2024");
  ok("e2e: month netR matches trade", Math.abs(stats.monthly[0].netR - tr.r) < 0.01);
}

// --- month-wise bucketing ---------------------------------------------------
{
  const mk = (iso, r, outcome) => ({
    ts: Date.parse(iso), r, outcome, direction: "bullish",
    tp1Hit: outcome === "win", tp2Hit: false, tp3Hit: false, slHit: outcome === "loss",
  });
  // IST = UTC+5:30, so 2024-02-29T20:00Z falls on 1 Mar IST -> March bucket
  const set = [
    mk("2024-01-05T10:00:00Z", 2, "win"),
    mk("2024-01-20T10:00:00Z", -1, "loss"),
    mk("2024-02-10T10:00:00Z", -1, "loss"),
    mk("2024-02-14T10:00:00Z", -1, "loss"),
    mk("2024-02-29T20:00:00Z", 3, "win"),
  ];
  const s = computeStats(set, fibConfig({ initialCapital: 10000, riskPercent: 1 }));
  ok("monthly: 3 buckets (IST rolls last trade to Mar)", s.monthly.length === 3);
  ok("monthly: keys sorted", s.monthly.map((m) => m.month).join(",") === "2024-01,2024-02,2024-03");
  const jan = s.monthly[0], feb = s.monthly[1], mar = s.monthly[2];
  ok("monthly: Jan 1W/1L", jan.wins === 1 && jan.losses === 1 && jan.winRate === 50);
  ok("monthly: Jan netR +1", jan.netR === 1);
  ok("monthly: Feb both losses", feb.wins === 0 && feb.losses === 2 && feb.netR === -2);
  ok("monthly: Feb negative return", feb.netProfit < 0 && feb.returnPct < 0);
  ok("monthly: Mar single win", mar.wins === 1 && mar.netR === 3);
  ok("monthly: profitable count", s.monthsProfitable === 2 && s.monthsLosing === 1);
  ok("monthly: best is Mar", s.bestMonth.month === "2024-03");
  ok("monthly: worst is Feb", s.worstMonth.month === "2024-02");
  ok("monthly: netR sums to total", Math.abs(s.monthly.reduce((a, m) => a + m.netR, 0) - s.netR) < 1e-9);
  ok("monthly: profits sum to total", Math.abs(s.monthly.reduce((a, m) => a + m.netProfit, 0) - s.netProfit) < 0.05);
  ok("monthly: months chain equity", Math.abs(feb.startEquity - jan.endEquity) < 0.05);
  ok("monthly: final = last endEquity", Math.abs(mar.endEquity - s.finalEquity) < 0.05);
  ok("monthly: Feb drawdown > 0", feb.maxDrawdown > 0);
}

console.log(`\n${pass} passed, ${fail} failed`);
process.exit(fail ? 1 : 0);
