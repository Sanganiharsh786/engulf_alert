...
// Real-time forex data for XAU/USD (Gold) and GBP/USD.
//
// Free tier: 800 requests/day, 8 requests/min.
// 2 symbols per scan → ~400 scans/day → scan every ~3 minutes.
//
...
export async function fetchTwelveData(tf = "4h", count = 100) {
  ...
  // Both pairs in a single batch request (URL-encode slashes for batch)
  const symbols = ["XAU/USD", "GBP/USD"]
  ...
  for (const symbol of ["XAU/USD", "GBP/USD"]) {
