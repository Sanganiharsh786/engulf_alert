...
export function computePosition({
  ...
  // standard forex lot-size formula (FundingPips):
  //   Lot = Amount at risk / (SL pips x pip value per lot)
  // Leverage does NOT change the lot size — only the margin required.
  const lots = riskAmount / (slPips * pipValuePerLot);
  const units = lots * cs; // = riskAmount / stopDistance
  const notional = units * entry;
  const margin = notional / lev; // leverage only affects this
  const marginExceedsAccount = margin > accountSize;
  ...
  return {
    ...
    rewardAmount: round(riskAmount * rr, 2),
    riskPercent,
...
export function positionText(pos) {
  ...
  const lines = [
    ...
    `Lot size   : ${pos.lots} lots`,
    `  = risk ${pos.riskAmount} / (${pos.slPips} pips x ${pos.pipValuePerLot}/pip per lot)`,
  ...
  if (pos.marginExceedsAccount) {
    lines.push(`NOTE: required margin exceeds account — raise leverage or lower risk.`);
  }
  return lines.join("\n");
}
