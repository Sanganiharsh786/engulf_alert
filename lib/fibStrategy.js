...
// ---------------------------------------------------------------------------
// Session filter (§21). Times are UTC. Ranges may wrap past midnight.
// ---------------------------------------------------------------------------
...
function withinRange(mins, start, end) {
...
export function inSession(tsMs, cfg) {
  ...
  const london = withinRange(mins, 8 * 60, 17 * 60);
  const ny = withinRange(mins, 13 * 60, 22 * 60);
  if (mode === "LONDON") return london;
  if (mode === "NEWYORK") return ny;
  if (mode === "LONDON_NY") return london || ny;
  if (mode === "CUSTOM") {
    return withinRange(mins, hhmmToMinutes(cfg.customSessionStart), hhmmToMinutes(cfg.customSessionEnd));
  }
  return true;
}
