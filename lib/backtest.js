...
export function filterTrades(trades, { from, to, pairs } = {}) {
...
const MAX_BARS = 12000; // safety cap (~125 days of 15m)

export async function backtestPair(pair, settings, { days = 10, bars } = {}) {
  ...
  const raw = await fetchOHLCV(pair, tf, nBars);
